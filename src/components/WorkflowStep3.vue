      ...
      <div class="mb-8">
        ...
        <div class="bg-gray-50 rounded-lg p-6">
          ...
          <div class="mb-6">
            ...
            <div
              v-if="hasTranscription && sourceOption === 'step2'"
              class="ml-7"
            >
              <div class="bg-white border rounded-lg p-3">
                ...
                <div class="text-sm text-gray-600">
                  <pre
                    v-if="showStep2Preview"
                    class="whitespace-pre-wrap font-mono max-h-32 overflow-y-auto"
                    >{{ workflowState.artifacts.transcriptionSRT }}</pre
                  >
            ...
            <div v-if="!hasTranscription" class="ml-7 text-sm text-gray-500">
              No subtitles available from Step 2. Complete Step 2 first or
          ...
          <div>
            ...
            <div v-if="sourceOption === 'upload'" class="ml-7">
              <p class="text-sm text-gray-600 mb-3">
                Upload your own SRT subtitle file to translate. Supports SRT
        ...
        <div class="bg-gray-50 rounded-lg p-6">
          ...
          >
            <div class="flex justify-between text-sm font-medium mb-2">
              <span>{{
                translationProgressObj.message || 'Preparing translation...'
              }}</span>
              <span class="text-green-600"
                >{{ translationProgressObj.progress }}%</span
              >
            </div>
            <div class="w-full bg-gray-300 rounded-full h-4 shadow-inner">
              <div
                ...
                :style="{ width: translationProgressObj.progress + '%' }"
              ></div>
            </div>
            <div
              v-if="
                translationProgressObj.currentSegment &&
                translationProgressObj.totalSegments
              "
              class="text-xs text-gray-600 mt-1"
            >
              Segment {{ translationProgressObj.currentSegment }} of
              {{ translationProgressObj.totalSegments }}
            </div>
...
Translated subtitle text here"
          ...
          <div class="p-4 border-t bg-blue-50">
            <details class="text-sm">
              ...
              <div class="text-blue-700 space-y-1 text-xs">
                <p>
                  • <strong>Format:</strong> Maintain SRT structure: number,
                  timestamps, and translated text
                </p>
                <p>
                  • <strong>Timestamps:</strong> Keep original timing:
                  00:00:00,000 --> 00:00:05,000
                </p>
                <p>
                  • <strong>Shortcuts:</strong> Ctrl+S to save, Show/Hide
                ...
                <p>
                  • <strong>Validation:</strong> Check for proper SRT format and
                  segment count
                </p>
              </div>
            </details>
      ...
      <!-- Next Step Button -->
      <div v-if="translatedSRT" class="flex justify-end mt-8">
        <button
          @click="goToStep4"
...
const sourceOption = ref<string>(
  workflowState.artifacts.transcriptionSRT ? 'step2' : 'upload'
)
...
const translationProgressObj = ref<{
  progress: number
...
onMounted(() => {
  ...
  if (workflowState.artifacts.translationSegments) {
    translationSegments.value = workflowState.artifacts.translationSegments.map(
      s => ({
        ...s,
        timestamp: [s.timestamp[0], s.timestamp[1]] as [number, number],
      })
  ...
  try {
    ...
    if (
      ...
      validationErrors.value.push(
        `Segment count mismatch: ${segments.length} translated vs ${originalSegments.length} original`
      )
    }
    ...
    validationErrors.value.push(
      'Failed to parse translation: Invalid SRT format'
    )
  }
}
...
function formatTranslation() {
  try {
    const segments = parseSRT(translatedSRT.value)
    if (Array.isArray(segments) && segments.length > 0) {
      const formattedSRT = generateTranslatedSRT(
        segments,
...
async function startTranslation() {
  ...
  try {
    ...
    translationError.value = ''
    translationProgressObj.value = {
      progress: 0,
      message: 'Preparing translation...',
    }

    console.log('📊 Translation progress bar state:', {
      isTranslating: isTranslating.value,
      progress: translationProgressObj.value.progress,
      status: translationProgressObj.value.message,
    })

    // Force DOM update to show progress bar immediately
    await nextTick()
    console.log(
      '🔄 DOM updated, translation progress bar should be visible now'
    )

    // Add a small delay to ensure the UI renders
    await new Promise(resolve => setTimeout(resolve, 100))
    console.log('⏱️ Translation initial delay complete')
    ...
    // Force DOM update after initial progress
    await nextTick()

    // Progress callbacks - let the translation service handle progress
    const onModelLoad = (progress: number | { progress: number }) => {
      isModelLoading.value = true
      const percent =
        typeof progress === 'number' ? progress : progress.progress
      modelLoadProgress.value = percent
      translationProgressObj.value = {
        progress: Math.round(percent * 0.3),
        message: `Loading ${availableModel.value?.name} model...`,
      }
    }

    const onTranslationProgress = (
      progress:
        | number
        | {
            progress: number
      ...
      let percent = 0
      let message = ''
      let currentSegment, totalSegments
      if (typeof progress === 'number') {
        percent = 30 + Math.round(progress * 0.7)
        message = `Translating subtitles... ${Math.round(progress)}%`
      } else {
        percent = 30 + Math.round((progress.progress ?? 0) * 0.7)
        message =
          progress.message ||
          `Translating subtitles... ${Math.round(progress.progress ?? 0)}%`
        currentSegment = progress.currentSegment
        totalSegments = progress.totalSegments
      }
      translationProgressObj.value = {
        progress: Math.max(translationProgressObj.value.progress, percent),
        message,
        currentSegment,
        totalSegments,
      }
    ...
    try {
      ...
      console.log(
        '⏰ Keeping translation progress bar visible for 2 seconds...'
      )
